...
//            System.out.println("ID: " + member.id + ", leader: " + member.leader.id + ", success, subtask " + member.mySubTask );
    ...
    private void renewRel(Agent agent){
        // 信頼度の更新式を書く. 下のは信頼度順に信頼エージェントを整列している
        Collections.sort(agent.relAgents, new AgentComparator());
        Collections.sort(agent.rel, new AgentComparator());
    }
